...
def fetch_weibo_data(keyword, start_time, end_time, page_id,cookie):
    ...
    all_contents = soup.select('.card-wrap')# 页面的全部内容
    ...
    for card in all_contents:
        if (card.get('mid') != None):# 如果微博ID不为空则开始抓取
            ...
            s = card.select_one('.txt').select_one('a[action-type="fl_unfold"]')# 是否需要展开全文
            ...
            if (s is None):
                ...
                # 获取展开全文后的微博
                wb_content = card.select('.txt')[1].text.replace('\n', '').replace(' ', '').replace('\r','').replace('收起全文d','').strip()  # 微博内容
            ...
            blog = {'wb_id': wb_id,  # 生成一条微博记录的列表
                    'wb_username': wb_username,  # 用户名
                    'wb_userid': wb_userid,  # 用户id
                    'wb_content': wb_content,  # 博文内容
                    'wb_createtime': wb_createtime,  # 发博时间
                    'wb_forwardnum': wb_forwardnum,  # 转发数
                    'wb_commentnum': wb_commentnum,  # 评论数
                    'wb_likenum': wb_likenum,  # 点赞数
                    'wb_url': wb_url  # url
                    }
            mblog.append(blog)
...
def fetch_pages(keyword, start_time, end_time,cookie):
    ...
    soup = BeautifulSoup(resp.text, 'lxml')
    # print(soup)
    t2 = soup.select_one('.card-wrap').select_one('p').text  # 选中第一条微博内容
    if (str(t2).startswith('抱歉')):  # 此次搜索条件的判断，如果没有相关搜索结果！退出...
        print("此次搜索条件无相关搜索结果！\n请重新选择条件筛选...")
        return
    else:
        ...
        for page_id in range(page_num):
            ...
            try:
                ...
                print(e)
        # 写入csv
    with open('data.csv', 'a+', newline='', encoding='utf-8-sig') as csvfile:  # 打开待写入csv
        writer = csv.writer(csvfile)
        for i in range(len(mblogs)):  # 循环写入爬取内容
            row = [keyword, mblogs[i]['wb_username'], mblogs[i]['wb_userid'],
...
if __name__ == '__main__':
    ...
    with open('data.csv', 'a+', newline='', encoding='utf-8-sig') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['关键词','用户名','用户id','博文内容','发博时间','转发数','评论数','点赞数','url'])
    i = 0
