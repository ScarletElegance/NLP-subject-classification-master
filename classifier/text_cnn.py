...
class TextCNN(object):
    ...
    def __init__(self, sequence_length, num_classes, vocab_size,
        ...
        # 定义占位符，分别输入输入集和标签集。在dropout层保持的神经元的数量也是网络的输入，因为我们可以只在训练过程中启用dropout，而评估模型的时候不启用。
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            self.W = tf.Variable(
                ...
                tf.random_uniform([vocab_size, embedding_size], -1.0, 1.0), name="W")#使用随机（未预先训练）的嵌入向量,词向量矩阵，初始时为均匀随机正态分布
            # 创建实际的embedding操作。embedding操作的结果是形状为 [None, sequence_length, embedding_size] 的3维张量积
            self.embedded_chars = tf.nn.embedding_lookup(self.W, self.input_x)#将输入序列向量化
        ...
        # Create a convolution + maxpool layer for each filter size.卷积和池化过程就是一个抽取特征的过程
        '''
